# Code for Figure 3 and (if applicable) corresponding statistical analyses for publication
...
jpeg("FigS2.1.jpg",width=17,height=17,units="cm",res= 600)
...
### Plot 3B: PCA plot with colored symbols labeling hybrid classes

# Define hybrid classes
head(dat)
levels(dat$HybClass_LDnigra)
...
head(hybID1)
gl1$pop <- factor(hybID1$HybClass_LDnigra,levels=c("jacea","BC2_jacea","BC1_jacea","F2","BC1_nigra", "BC2_nigra", "nigra")) 
gl1$pop[1:3]

hybCode1 <-factor(hybID1$HybClass_LDnigra,levels=c("jacea","BC2_jacea","BC1_jacea","F2","BC1_nigra", "BC2_nigra", "nigra"))
levels(gl1$pop)
...
### Plot 3C-D: Relate Admixture and hybrid classes to PC scores

# (C) PC1 and C. nigra cluster (K=2)

plot(dat$PC1,dat$AnK2_K1, ylab= "",xlab=""
     , ann=F
     ,pch=symbols[dat$HybClass_LDnigra],col=colors[dat$HybClass_LDnigra], bg=bg[dat$HybClass_LDnigra]
) 
mtext(side = 1, text = "Genetic principal component 1", line = 2.5,cex=0.7)
